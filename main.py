...
async def predict(file: UploadFile = File(...)):
    try:
        ...
        top_3_notes_idx = sorted_idxs[:3]
        lesser_7_notes_idx = sorted_idxs[3:10]

        top_notes = [get_note_name(idx) for idx in top_3_notes_idx]
        lesser_notes = [get_note_name(idx) for idx in lesser_7_notes_idx]

        # Predict chord based on top 3 notes
        predicted_chord, score = predict_chord(top_3_notes_idx.tolist())
